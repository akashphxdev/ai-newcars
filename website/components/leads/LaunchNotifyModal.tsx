...
// Clone of PriceDropAlertModal.tsx — same guest-OTP-vs-logged-in shell
// (via useLeadOtpFlow), just launch-dated copy instead of price-dated.
    ...
    >
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-border shadow-2xl">
        ...
        <div className="relative flex flex-col items-center gap-1 px-6 pb-2 pt-8 text-center">
          ...
          <h3 className="text-[16px] font-bold text-white">{done ? "You're all set!" : "Never Miss the Launch!"}</h3>
          {!done && (
            <p className="text-[12.5px] leading-relaxed text-white/80">
              Get notified the moment {carName} launches.
            </p>
          )}
        </div>
        ...
        <div className="relative px-6 pb-6 pt-3">
          {done ? (
            <p className="text-center text-[13px] leading-relaxed text-white/80">
              We&apos;ll email you the moment this car launches.
            </p>
          ) : step === "otp" ? (
            <form id="launch-notify-form" onSubmit={handleOtpSubmit} className="flex flex-col gap-3">
            ...
            <form id="launch-notify-form" onSubmit={handleFormSubmit} className="flex flex-col gap-3">
              {isLoggedIn ? (
