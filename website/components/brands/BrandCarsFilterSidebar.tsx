...
type Draft = { bodyType: string[]; fuelType: string[]; maxPriceLakh: number };
...
export default function BrandCarsFilterSidebar({
  brandSlug,
  bodyTypes,
  fuelTypes,
  initial,
  ...
  bodyTypes: BrandCarsBodyTypeFilter[];
  fuelTypes: BrandCarsFuelTypeFilter[];
  initial: { bodyType?: string[]; fuelType?: string[]; maxPrice?: number };
}) {
  ...
  }, [initial.bodyType?.join(","), initial.fuelType?.join(","), initial.maxPrice]);
  ...
  const reset = () => {
    setDraft({ bodyType: [], fuelType: [], maxPriceLakh: MAX_PRICE_LAKH });
  ...
  return (
    <>
      <aside className="sticky top-20 hidden h-fit max-h-[calc(100vh-6rem)] flex-col gap-5 self-start overflow-y-auto rounded-2xl border border-border bg-surface p-5 lg:flex">
        ...
        <FilterFields bodyTypes={bodyTypes} fuelTypes={fuelTypes} draft={draft} setDraft={setDraft} onApply={apply} onReset={reset} />
      </aside>
