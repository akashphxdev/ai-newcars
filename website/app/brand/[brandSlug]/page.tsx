...
// Reached as "/tata-cars" through the single catch-all rewrite in
// next.config.ts. Body types used to share this page and were told apart
// by querying brands first and falling through — every body-type render
// paid for a wasted brand lookup. They now live at /new-cars/[bodyType].
...
async function BrandCarsPageContent({ slug, sp }: { slug: string; sp: SearchParams }) {
  ...
  const basePath = routes.brand(brand.slug);
...
export default async function BrandCarsPage({ params, searchParams }: Props) {
  ...
  return <BrandCarsPageContent slug={brandSlug} sp={sp} />;
}
