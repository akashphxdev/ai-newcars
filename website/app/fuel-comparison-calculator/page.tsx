...
export default async function FuelComparisonCalculatorPage() {
  const [brands, seo] = await Promise.all([
    getAllBrands(),
    getSeoMeta({ pageType: SEO_PAGE_TYPE.STATIC, staticPageSlug: "fuel-comparison-calculator" }),
  ]);
    ...
    <div>
      ...
      <div className="border-b border-border bg-surface">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:py-10">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-ink sm:text-[28px]">{seo?.h1Tag || "Petrol vs Diesel vs CNG vs EV — Running Cost"}</h1>
