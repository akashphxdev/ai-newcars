// src/pages/BuyLeads/LoanLeads/LoanLeadExpandedDetail.tsx
//
// Mounted only while its row is expanded — same convention as
// BuyLeads/InsuranceLeads/InsuranceLeadExpandedDetail.tsx.
import { useState } from "react";
import { useGetLoanLeadByIdQuery, useAddLoanLeadActivityMutation, type LoanLeadRecord } from "./loanLead.api";
...
export default function LoanLeadExpandedDetail({ lead }: { lead: LoanLeadRecord }) {
  const { data: detail, isLoading, error: queryError } = useGetLoanLeadByIdQuery(lead.id);
  const error = queryError ? "Couldn't load lead detail." : "";

  const [note, setNote] = useState("");
  const [addActivity, { isLoading: posting }] = useAddLoanLeadActivityMutation();
  ...
  return (
    <div className="space-y-4 text-[12px]">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3">
        <Field label="Loan Amount" value={fmtAmount(detail.loanAmount)} />
        <Field label="Tenure" value={detail.tenureYears ? `${detail.tenureYears} Years` : "—"} />
        <Field label="Interest Rate" value={detail.interestRate ? `${Number(detail.interestRate).toFixed(2)}%` : "—"} />
        <Field label="Monthly Income" value={fmtAmount(detail.monthlyIncome)} />
