// src/pages/BuyLeads/SoftLeads/SoftLeadExpandedDetail.tsx
//
// Mounted only while its row is expanded — same convention as
// BuyLeads/PriceDropAlerts/PriceDropLeadExpandedDetail.tsx.
import { useState } from "react";
import { useGetSoftLeadByIdQuery, useAddSoftLeadActivityMutation, SOFT_LEAD_CALCULATOR_LABELS, type SoftLeadRecord } from "./softLead.api";
...
export default function SoftLeadExpandedDetail({ lead }: { lead: SoftLeadRecord }) {
  const { data: detail, isLoading, error: queryError } = useGetSoftLeadByIdQuery(lead.id);
  const error = queryError ? "Couldn't load lead detail." : "";

  const [note, setNote] = useState("");
  const [addActivity, { isLoading: posting }] = useAddSoftLeadActivityMutation();
  ...
  return (
    <div className="space-y-4 text-[12px]">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3">
        <Field label="Calculator" value={detail.calculatorType ? SOFT_LEAD_CALCULATOR_LABELS[detail.calculatorType] : "—"} />
        <Field label="Inputs" value={detail.inputSummary ?? "—"} />
