// src/pages/BuyLeads/NewCarLeads/AllNewCarLeads.tsx
import { useEffect, useState } from "react";
import {
  useGetBuyNewCarLeadsQuery,
  useGetBuyNewCarLeadStatsQuery,
  useUpdateBuyNewCarLeadStatusMutation,
  type BuyNewCarLeadRecord,
  type BuyNewCarLeadStatus,
  type BuyNewCarLeadInterestType,
} from "./buyNewCarLead.api";
...
import NewCarLeadExpandedDetail from "./NewCarLeadExpandedDetail";

const PAGE_SIZE_OPTIONS = [10, 20, 50, 100];

const STATUS_OPTIONS: BuyNewCarLeadStatus[] = ["new", "contacted", "qualified", "converted", "junk"];

const STATUS_STYLES: Record<BuyNewCarLeadStatus, { bg: string; text: string }> = {
...
const INTEREST_TYPE_LABELS: Record<BuyNewCarLeadInterestType, string> = {
  enquiry: "Enquire Now",
  offer_check: "Check Offers",
};
...
function StatusSelect({
  ...
  value: BuyNewCarLeadStatus;
  onChange: (next: BuyNewCarLeadStatus) => void;
  disabled?: boolean;
}) {
  ...
  return (
    <select
      value={value}
      disabled={disabled}
      onChange={(e) => onChange(e.target.value as BuyNewCarLeadStatus)}
...
export default function AllNewCarLeads() {
  ...
  const [status, setStatus] = useState<BuyNewCarLeadStatus | "">("");
  ...
  const { data: stats } = useGetBuyNewCarLeadStatsQuery();
  ...
  const [updateStatus] = useUpdateBuyNewCarLeadStatusMutation();
  const [togglingId, setTogglingId] = useState<number | null>(null);
  const [actionError, setActionError] = useState("");

  const handleStatusChange = async (lead: BuyNewCarLeadRecord, next: BuyNewCarLeadStatus) => {
    setActionError("");
  ...
  const columns: DataTableColumn<BuyNewCarLeadRecord>[] = [
    {
      header: "Lead",
      render: (r) => (
        <div>
    ...
    },
    {
      ...
      render: (r) => (
        <span className="text-[11px] font-semibold text-[#4a4640]">
          {r.interestType ? INTEREST_TYPE_LABELS[r.interestType] : "—"}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="space-y-5 max-w-[1400px]">
      <div>
        <h1 className="text-[18px] font-black text-[#1c1a17]">New Car Leads</h1>
        <p className="text-[12px] text-[#a39e96] mt-0.5">
          Buy-new-car enquiries submitted from car model pages on the website. Click a row for full detail.
        </p>
      </div>
      ...
      >
        ...
        <FilterSelect
          value={status}
          onChange={(v) => {
            setStatus((v as BuyNewCarLeadStatus) || "");
            setPage(1);
          }}
      ...
      <div className="bg-white border border-[#e8e4dc] rounded-xl overflow-hidden">
        <DataTable
          ...
          expandable
          renderExpanded={(r) => <NewCarLeadExpandedDetail lead={r} />}
        />
