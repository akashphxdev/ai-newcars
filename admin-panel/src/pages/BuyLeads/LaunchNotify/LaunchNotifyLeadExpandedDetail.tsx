// src/pages/BuyLeads/LaunchNotify/LaunchNotifyLeadExpandedDetail.tsx
//
...
export default function LaunchNotifyLeadExpandedDetail({ lead }: { lead: LaunchNotifyLeadRecord }) {
  const { data: detail, isLoading, error: queryError } = useGetLaunchNotifyLeadByIdQuery(lead.id);
  const error = queryError ? "Couldn't load lead detail." : "";

  const [note, setNote] = useState("");
  const [addActivity, { isLoading: posting }] = useAddLaunchNotifyLeadActivityMutation();
  ...
  return (
    <div className="space-y-4 text-[12px]">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-3">
        <Field label="Notified At" value={detail.notifiedAt ? formatDateTime(detail.notifiedAt) : "Not notified yet"} />
        <Field label="Model Launch Status" value={detail.model?.launchStatus ?? "—"} />
