...
import {
  SEO_PAGE_TYPE,
  STATIC_PAGE_PLACEHOLDER_TOKENS,
  ROBOTS_PRESETS,
  SCHEMA_FIELDS,
  type SchemaFieldKey,
} from "../../../lib/lookups";
...
function SeoMetaForm({ slug, slugLabel, seoMeta }: { slug: string; slugLabel: string; seoMeta: SeoMetaRecord | null }) {
  ...
  const placeholderTokens = STATIC_PAGE_PLACEHOLDER_TOKENS[slug] ?? [];
      ...
      <form onSubmit={handleSubmit} className="px-6 pb-6 pt-5 space-y-4" noValidate>
        {placeholderTokens.length > 0 && (
          <div className="rounded-xl border border-[#f0d9d2] bg-[#fefaf9] px-3.5 py-2.5">
            <p className="text-[11.5px] font-bold text-[#1c1a17]">This page is a shared template</p>
            <p className="text-[11px] text-[#8a8579] mt-0.5 leading-snug">
              It's reused for every comparison (2-4 cars), so write it with the tokens below instead of specific car names —
              the website fills them in with the actual cars being compared. car3/car4 tokens are safe to use even for a
              2-car pair; they're automatically removed when there's no 3rd/4th car.
            </p>
          </div>
        )}
        {/* ---------------- Meta Info tab ---------------- */}
        <div className={activeTab === "meta" ? "space-y-4" : "hidden"}>
          <SectionCard title="Basic Info" icon={<InfoSectionIcon />}>
            ...
            <Field label="Meta keywords" hint="Optional, comma-separated.">
              <TextField value={metaKeywords} onChange={setMetaKeywords} placeholder="e.g. maruti suzuki, cars, price" tokens={placeholderTokens} />
            </Field>

            <Field label="H1 tag" hint="Overrides this page's on-page heading. Falls back to the page's own default if blank.">
              <TextField value={h1Tag} onChange={setH1Tag} placeholder="e.g. Maruti Suzuki Cars in India" maxLength={255} tokens={placeholderTokens} />
            </Field>
          </SectionCard>
          ...
          <SectionCard title="Search Engine" icon={<SearchEngineSectionIcon />}>
            <Field
              ...
              hint={
                slug === "compare-detail"
                  ? "Ignored for this page — each comparison pair always canonicalizes to its own URL."
                  : undefined
              }
            >
