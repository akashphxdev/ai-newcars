...
// Landing-page files, different from every other upload here in one way
// that matters: the filename must survive.
//
// A campaign page references its image or stylesheet by the name the
// designer used ("scorpio-hero.jpg", "css/site.css"). buildUploader
// renames every file to a timestamp and withAvifConversion rewrites
// images to .avif — either would leave the page pointing at a file that
// no longer exists. So this keeps the original name, does no conversion,
// and hands the bytes over in memory for the service to place. What is
// an acceptable file type is decided there, next to the path checks.
const LANDING_FILE_MAX_BYTES = 16 * 1024 * 1024;

export const landingAssetUploader = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: LANDING_FILE_MAX_BYTES, files: 60 },
}).array('files', 60);
