  ...
  const powertrain = await prisma.$transaction(async (tx) => {
    ...
    return tx.carPowertrainElectric.create({
      data: {
        ...
        chargingPort: input.chargingPort,
        chargingOptionsRaw: input.chargingOptionsRaw,
        regenerativeBraking: input.regenerativeBraking,
        regenerativeBrakingLevels: input.regenerativeBrakingLevels,
