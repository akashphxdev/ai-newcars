...
async function getCarWithVariant(slug: string, variantId?: number, powertrainId?: number): Promise<CompareCarResult> {
  ...
  const specs: CompareCarSpecs = {
    ...
    emissionNormCompliance: ice?.emissionNormCompliance ?? electric?.emissionNormCompliance ?? null,
    turboCharger: ice?.turboCharger ?? false,
    ...
    chargingPort: electric?.chargingPort ?? null,
    chargingOptionsRaw: electric?.chargingOptionsRaw ?? null,
    regenerativeBraking: electric?.regenerativeBraking ?? false,
    regenerativeBrakingLevels: electric?.regenerativeBrakingLevels ?? null,
